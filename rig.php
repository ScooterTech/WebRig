...
function rigstart(){
    ...
    gmainsub = (getCookie('gmainsub') == "true");
    if (gmainsub)
        document.getElementById("vfo").innerHTML = "MAIN";
    else
        document.getElementById("vfo").innerHTML = "VFOA";

    poll();
    if (refresh != "OFF") polltimer = setInterval("poll()", refresh);
    
//    loadXMLDoc("rigctl.php?q=1", getcaps);
}
...
function setvfo(){
    if (gmainsub){
        if (gvfo == "MAIN") gvfo = "SUB";
        else gvfo = "MAIN";
    }else{
        if (gvfo == "VFOA") gvfo = "VFOB";
        else gvfo = "VFOA";
    }
    document.getElementById("vfo").innerHTML = gvfo;
...
function setmainsub(){
    mainsub = document.getElementById('mainsub').value;
    setCookie('mainsub', mainsub, 366);
    gmainsub = (mainsub == "true");
}
...
?>    
    ...
    <div id="divopts">
      ...
      <tr class="odd"><td class="right">VFO set:</td><td>
      <select id="mainsub" onchange="setmainsub()">
          <option value="false">A/B</option>
          <option value="true">Main/Sub</option>
      </select>         
      </td></tr>
      <tr><td colspan="2"><hr></td></tr>
